...
export default function SchedulePage() {
    ...
    const [publishedPlatformFilter, setPublishedPlatformFilter] = useState<Platform | 'all'>('all');
    const [publishedTypeFilter, setPublishedTypeFilter] = useState<PostType | 'all'>('all');
    ...
    useEffect(() => {
        function handleClickOutside(e: MouseEvent) {
            if (publishedFilterRef.current && !publishedFilterRef.current.contains(e.target as Node)) {
                setIsPublishedFilterOpen(false);
            }
        }
        if (isPublishedFilterOpen) document.addEventListener('mousedown', handleClickOutside);
        return () => document.removeEventListener('mousedown', handleClickOutside);
    ...
    const filteredPublishedPosts = MOCK_PUBLISHED.filter((post) => {
        const matchesPlatform = publishedPlatformFilter === 'all' || post.platform === publishedPlatformFilter;
        const matchesType = publishedTypeFilter === 'all' || post.postType === publishedTypeFilter;
        return matchesPlatform && matchesType;
    });
                ...
                <div>
                    ...
                    {MOCK_PUBLISHED.length > 0 && (
                        <div className="flex items-center justify-between mb-5">
                            ...
                            <div className="flex items-center gap-1 bg-gray-50 rounded-lg p-1">
                                <button
                                    onClick={() => setPublishedTypeFilter('all')}
                                    className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${
                                ...
                                {POST_TYPES.map((t) => (
                                    <button
                                        key={t.id}
                                        onClick={() => setPublishedTypeFilter(t.id)}
                                        className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${
                            ...
                            <div className="relative" ref={publishedFilterRef}>
                                <button
                                    onClick={() => setIsPublishedFilterOpen(!isPublishedFilterOpen)}
                                    className={`flex items-center gap-2 px-3 py-1.5 text-sm border rounded-lg transition-colors ${
                                ...
                                >
                                    ...
                                    {publishedPlatformFilter === 'all'
                                        ? 'Filter'
                                        : PLATFORMS.find((p) => p.id === publishedPlatformFilter)?.name}
                                </button>

                                {isPublishedFilterOpen && (
                                    <div className="absolute right-0 top-full mt-1 w-44 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10">
                                        <button
                                            onClick={() => { setPublishedPlatformFilter('all'); setIsPublishedFilterOpen(false); }}
                                            className={`flex items-center justify-between w-full px-3 py-2 text-sm transition-colors ${
                                                publishedPlatformFilter === 'all'
                                                    ? 'bg-blue-50 text-blue-600'
                                                    : 'text-gray-600 hover:bg-gray-50'
                                        ...
                                        >
                                            All Platforms
                                            {publishedPlatformFilter === 'all' && <Check size={14} />}
                                        </button>
                                        {PLATFORMS.map((p) => (
                                            <button
                                                key={p.id}
                                                onClick={() => { setPublishedPlatformFilter(p.id); setIsPublishedFilterOpen(false); }}
                                                className={`flex items-center justify-between w-full px-3 py-2 text-sm transition-colors ${
                                                    publishedPlatformFilter === p.id
                                                        ? 'bg-blue-50 text-blue-600'
                                                        : 'text-gray-600 hover:bg-gray-50'
                                                }`}
                                            >
                                                ...
                                                {publishedPlatformFilter === p.id && <Check size={14} />}
                                            </button>
                                        ))}
                                    </div>
                                )}
                    ...
                    {filteredPublishedPosts.length > 0 ? (
                        ...
                        <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
                            ...
                            <button
                                onClick={() => { setPublishedPlatformFilter('all'); setPublishedTypeFilter('all'); }}
                                className="px-4 py-2 text-sm text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
                            >
                                Clear filters
                            </button>
